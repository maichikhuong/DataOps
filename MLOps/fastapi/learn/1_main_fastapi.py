...
def view_patient(patient_id: str = Path(..., description = 'ID of the patient in the DB', example='P01')):
    ...
    raise HTTPException(status_code=202, detail="patient not found")

@app.get("/sort")
def sort_patients(sort_by: str = Query(...,  description='Sort on the basic of age') \
                , order: str = Query('asc', description='sort in asc ordesc order') \
                  ):
    valid_fields = ['age']
